...
假设你在使用一个终端窗口登录到系统，你启动了一个文件复制任务（进程组1）和一个文件搜索任务（进程组2）。这两个进程组都属于同一个会话，并且与你的控制终端关联。这样，你可以在终端中实时查看复制进度并与文件搜索任务进行交互。如果你注销登录，整个会话（包括两个进程组和控制终端）都会被终止。
...
- server验证client身份，同样需要将client public key添加到authorized_keys中。
